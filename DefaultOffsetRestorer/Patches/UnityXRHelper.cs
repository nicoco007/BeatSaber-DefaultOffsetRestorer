...
namespace DefaultOffsetRestorer.Patches
{
    ...
    /// so the offsets result in the same position as in Beat Saber before the OpenXR update.
    /// </summary>
    ...
    internal static class UnityXRHelper_Start
    {
        ...
        private static readonly Pose kOriginalHtcViveOffset = new(new Vector3(0f, -0.008f, 0f), Quaternion.Euler(-4.3f, 0f, 0f));
        private static readonly Pose kOriginalOculusOffset = new(new Vector3(0f, 0f, 0.055f), Quaternion.Euler(-40f, 0f, 0f)); // TODO: this is from OculusVRHelper; might be wrong
        private static readonly Pose kOriginalKnucklesOffset = new(new Vector3(0f, 0.022f, -0.01f), Quaternion.Euler(-16.3f, 0f, 0f));

        private static void Prefix(UnityXRHelper __instance)
        {
            __instance._htcVivePoseOffset = AddOffsets(kHtcViveGripOffset, kOriginalHtcViveOffset);
            __instance._oculusPoseOffset = AddOffsets(kOculusGripOffset, kOriginalOculusOffset);
            __instance._valveIndexPoseOffset = AddOffsets(kKnucklesGripOffset, kOriginalKnucklesOffset);
        }

        private static EulerPose AddOffsets(Pose gripOffset, Pose originalOffset)
        {
            Vector3 localPosition = (Quaternion.Inverse(gripOffset.rotation) * -gripOffset.position) + (Quaternion.Inverse(gripOffset.rotation) * originalOffset.rotation * originalOffset.position);
            Quaternion localRotation = Quaternion.Inverse(gripOffset.rotation) * originalOffset.rotation;
